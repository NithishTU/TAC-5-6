...
# Claude uses local authentication - run 'claude auth login' first
export CLAUDE_CODE_PATH="/path/to/claude"  # Optional, defaults to "claude"
export GITHUB_PAT="ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"  # Optional, only if using different account than 'gh auth login'
```

**Important:** ADW now uses local Claude authentication instead of API keys. Before running ADW:
...
# Verify Claude authentication (required for Claude Max subscription)
claude auth status
